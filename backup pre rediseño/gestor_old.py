...
def get_db_connection():
    # Read DB credentials from environment first
            ...
            for candidate in glob.glob('/var/www/*_wsgi.py'):
                try:
                    ...
                    for var in ('DB_HOST', 'DB_NAME', 'DB_USER', 'DB_PASS', 'SECRET_KEY'):
                        ...
                        if m and os.getenv(var) is None:
    ...
    print('Credenciales de MySQL ausentes. No se puede conectar.')
    return None
...
def init_db():
    ...
    try:
        ...
        print(f"\nERROR al inicializar la BD (no-MySQL): {e}")
...
def agregar_producto():
    ...
    proveedores_excel_list = [ {'key': p['nombre'].lower().replace(' ', '_'), 'nombre': p['nombre']} for p in proveedores_manual ]
...
def proveedores():
    ...
    if busqueda_proveedor:
        ...
        proveedores_manual = db_query("SELECT * FROM proveedores_manual WHERE nombre LIKE %s ORDER BY nombre", (like_pattern,), fetch=True)
    else:
        proveedores_manual = db_query("SELECT * FROM proveedores_manual ORDER BY nombre", fetch=True)
    return render_template('proveedores.html', proveedores_manual=proveedores_manual, busqueda_proveedor=busqueda_proveedor)
    ...
    if request.method == 'POST' and 'buscar' in request.form:
        if os.path.exists(MANUAL_PRODUCTS_FILE):
            try:
                ...
                # Buscar por nombre, código, precio o proveedor
                productos_encontrados = df[
                    df['Nombre'].astype(str).str.contains(search_term, case=False, na=False) |
                    df['Codigo'].astype(str).str.contains(search_term, case=False, na=False) |
                    df['Precio'].astype(str).str.contains(search_term, case=False, na=False) |
                    df['Proveedor'].astype(str).str.contains(search_term, case=False, na=False)
                ].to_dict('records')
            except Exception as e:
                flash(f"Error al buscar en el archivo Excel: {e}", 'danger')
        else:
        ...
        if os.path.exists(MANUAL_PRODUCTS_FILE):
            try:
                import pandas as pd
                df = pd.read_excel(MANUAL_PRODUCTS_FILE)
                df = df[df['Codigo'].astype(str) != str(codigo_a_eliminar)]
                df.to_excel(MANUAL_PRODUCTS_FILE, index=False)
                flash('Producto eliminado correctamente del archivo manual.', 'success')
            except Exception as e:
                flash(f"Error al eliminar en el archivo Excel: {e}", 'danger')
...
def eliminar_manual_ajax():
    codigo_a_eliminar = request.json.get('codigo_a_eliminar')
    ...
    if os.path.exists(MANUAL_PRODUCTS_FILE):
        try:
            ...
            df = df[df['Codigo'].astype(str) != str(codigo_a_eliminar)]
            df.to_excel(MANUAL_PRODUCTS_FILE, index=False)
            msg = 'Producto eliminado correctamente del archivo manual.'
            # Actualizar resultados con el mismo término de búsqueda
            productos_encontrados = df[
                df['Nombre'].astype(str).str.contains(search_term, case=False, na=False) |
                df['Codigo'].astype(str).str.contains(search_term, case=False, na=False) |
                df['Precio'].astype(str).str.contains(search_term, case=False, na=False) |
            ...
            return jsonify({'success': False, 'msg': f'Error al eliminar: {e}', 'productos': []})
    else:
        return jsonify({'success': False, 'msg': 'El archivo de productos manuales no existe.', 'productos': []})
    # Renderizar solo la tabla de resultados
    html = render_template('eliminar_manual_resultados.html', productos=productos_encontrados, search_term=search_term)
    return jsonify({'success': True, 'msg': msg, 'html': html})
...
def eliminar_manual_seleccionados_ajax():
    ...
    if os.path.exists(MANUAL_PRODUCTS_FILE):
        try:
            df = pd.read_excel(MANUAL_PRODUCTS_FILE)
            df = df[~df['Codigo'].astype(str).isin([str(c) for c in codigos])]
            df.to_excel(MANUAL_PRODUCTS_FILE, index=False)
            msg = f'{len(codigos)} producto(s) eliminados correctamente.'
            productos_encontrados = df[
                df['Nombre'].astype(str).str.contains(search_term, case=False, na=False) |
                df['Codigo'].astype(str).str.contains(search_term, case=False, na=False) |
                df['Precio'].astype(str).str.contains(search_term, case=False, na=False) |
...
def eliminar_manual_todo_ajax():
    ...
    if os.path.exists(MANUAL_PRODUCTS_FILE):
        try:
            df = pd.read_excel(MANUAL_PRODUCTS_FILE)
            # Identificar los productos que coinciden con la búsqueda
            mask = (
                df['Nombre'].astype(str).str.contains(search_term, case=False, na=False) |
                df['Codigo'].astype(str).str.contains(search_term, case=False, na=False) |
                df['Precio'].astype(str).str.contains(search_term, case=False, na=False) |
            ...
            encontrados = df[mask]
            restantes = df[~mask]
            restantes.to_excel(MANUAL_PRODUCTS_FILE, index=False)
            msg = f"{len(encontrados)} producto(s) eliminados de la búsqueda." if len(encontrados) > 0 else "No se encontraron productos para eliminar."
            productos_encontrados = []
        except Exception as e:
            return jsonify({'success': False, 'msg': f'Error al eliminar: {e}', 'html': ''})
    else:
        return jsonify({'success': False, 'msg': 'El archivo de productos manuales no existe.', 'html': ''})
    html = render_template('eliminar_manual_resultados.html', productos=productos_encontrados, search_term=search_term)
    return jsonify({'success': True, 'msg': msg, 'html': html})

# --- Funciones de Lógica de Negocio ---
def buscar_en_excel(termino_busqueda, proveedor_filtro=None, filtro_adicional=None):
    # Obtener todos los proveedores manuales
    proveedores_manual = db_query("SELECT * FROM proveedores_manual ORDER BY nombre", fetch=True) or []
    proveedores_manual_nombres = [p['nombre'].lower().replace(' ', '_') for p in proveedores_manual]
    PROVEEDOR_CONFIG = {
    ...
    archivos_a_buscar = []
    if proveedor_filtro:
        proveedor_filtro_key = proveedor_filtro.lower().replace(' ', '_')
        # Si el proveedor está en la lista de proveedores manuales, buscar en productos_manual.xlsx
        if proveedor_filtro_key in proveedores_manual_nombres:
            if os.path.exists(MANUAL_PRODUCTS_FILE):
                archivos_a_buscar.append(MANUAL_PRODUCTS_FILE)
                proveedor_manual_nombre = next((p['nombre'] for p in proveedores_manual if p['nombre'].lower().replace(' ', '_') == proveedor_filtro_key), None)
        elif proveedor_filtro_key in PROVEEDOR_CONFIG:
            config = PROVEEDOR_CONFIG[proveedor_filtro_key]
            if 'archivo_excel' in config:
                ...
                archivos_a_buscar = [f for f in all_files if proveedor_filtro_key in os.path.basename(f).lower()]
        else:
            archivos_a_buscar = []
    else:
    ...
    for archivo in archivos_a_buscar:
        try:
            ...
            if not col_codigo or not col_producto:
                ...
                continue
            # Si es productos_manual.xlsx y hay proveedor_manual_nombre, filtrar por proveedor
            if archivo == MANUAL_PRODUCTS_FILE and 'proveedor_manual_nombre' in locals() and proveedor_manual_nombre:
                df = df[df['Proveedor'].astype(str).str.lower() == proveedor_manual_nombre.lower()]
...
def agregar_proveedor_manual_ajax():
    ...
    db_query("INSERT IGNORE INTO proveedores_manual (nombre) VALUES (%s)", (nombre,))
    proveedores = db_query("SELECT * FROM proveedores_manual ORDER BY nombre", fetch=True)
...
def eliminar_seleccionados():
    # Recoger todos los valores enviados con el nombre 'seleccionados'
    raw_ids = request.form.getlist('seleccionados')
    cleaned_ids = []
    for val in raw_ids:
        if val is None:
            continue
        # Si vienen en un solo string separado por comas, dividir
        parts = str(val).split(',')
        for p in parts:
            p = p.strip()
            if not p:
                continue
            # intentar convertir a entero (ignorar no-numéricos)
            try:
                cleaned_ids.append(int(p))
            except Exception:
                # si no es entero, aceptarlo como string id (fallback)
    ...
    if unique_ids:
        for id in unique_ids:
            try:
                ...
                pass
        flash(f"{len(unique_ids)} productos eliminados del historial.", 'success')
    else:
...
def gestionar_manual_actualizar_ajax():
    ...
    if os.path.exists(MANUAL_PRODUCTS_FILE):
        try:
            df = pd.read_excel(MANUAL_PRODUCTS_FILE)
            idx = df[df['Codigo'].astype(str) == str(codigo_original)].index
            if len(idx) == 0:
                return jsonify({'success': False, 'msg': 'Producto no encontrado.', 'html': ''})
            ...
            producto_actualizado = df.loc[idx[0]].to_dict()
            html = render_template('editar_manual_formulario.html', producto=producto_actualizado)
        except Exception as e:
            return jsonify({'success': False, 'msg': f'Error al actualizar: {e}', 'html': ''})
    else:
        return jsonify({'success': False, 'msg': 'El archivo de productos manuales no existe.', 'html': ''})
    return jsonify({'success': True, 'msg': msg, 'html': html})
...
def gestionar_manual_buscar_ajax():
    ...
    if not valor:
        return jsonify({'html': '<div class="alert alert-danger">Debes ingresar un valor.</div>'})
    if os.path.exists(MANUAL_PRODUCTS_FILE):
        try:
            df = pd.read_excel(MANUAL_PRODUCTS_FILE)
            producto = df[
                df['Codigo'].astype(str).str.contains(valor, case=False, na=False) |
                df['Nombre'].astype(str).str.contains(valor, case=False, na=False) |
                df['Proveedor'].astype(str).str.contains(valor, case=False, na=False) |
                df['Precio'].astype(str).str.contains(valor, case=False, na=False)
            ]
            if producto.empty:
                html = render_template('editar_manual_formulario.html', error='No se encontró el producto con ese valor.')
            else:
                html = render_template('editar_manual_formulario.html', producto=producto.iloc[0].to_dict())
            return jsonify({'html': html})
        except Exception as e:
            return jsonify({'html': f'<div class="alert alert-danger">Error: {e}</div>'})
    else:
        return jsonify({'html': '<div class="alert alert-danger">El archivo de productos manuales no existe.</div>'})

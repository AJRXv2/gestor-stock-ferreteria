...
def init_excel_manual():
    ...
    if not os.path.exists(MANUAL_PRODUCTS_FILE):
        ...
        # Crear encabezados - estructura corregida
        headers = ['Codigo', 'Proveedor', 'Nombre', 'Precio', 'PrecioTexto', 'Observaciones', 'Dueno']
...
def agregar_producto_excel_manual(codigo, proveedor, nombre, precio, observaciones, dueno):
    ...
    try:
        init_excel_manual()  # Asegurar que el archivo existe
        
        # Abrir el archivo Excel
        wb = openpyxl.load_workbook(MANUAL_PRODUCTS_FILE)
        ws = wb.active
        
        # Encontrar la próxima fila vacía
        next_row = ws.max_row + 1
        
        # Agregar el producto con la estructura correcta
        ws.cell(row=next_row, column=1, value=codigo or '')  # Codigo
        ws.cell(row=next_row, column=2, value=proveedor or '')  # Proveedor
        ws.cell(row=next_row, column=3, value=nombre or '')  # Nombre
        ws.cell(row=next_row, column=4, value=precio or 0)  # Precio
        ws.cell(row=next_row, column=5, value=str(precio) if precio else '')  # PrecioTexto
        ws.cell(row=next_row, column=6, value=observaciones or '')  # Observaciones
        ws.cell(row=next_row, column=7, value=dueno or '')  # Dueno
        
        wb.save(MANUAL_PRODUCTS_FILE)
        return True
    except Exception as e:
        print(f"Error al agregar producto al Excel: {e}")
        return False
...
def buscar_en_excel_manual(termino_busqueda, dueno_filtro=None):
    ...
    try:
        if not os.path.exists(MANUAL_PRODUCTS_FILE):
            print(f"Archivo Excel no existe: {MANUAL_PRODUCTS_FILE}")
            return resultados
        ...
        # Verificar si existe la columna Dueno, si no existe, crear una por defecto
        if 'Dueno' not in df.columns:
            print("Columna 'Dueno' no encontrada, asignando 'ferreteria_general' por defecto")
            df['Dueno'] = 'ferreteria_general'
        
        # Filtrar por término de búsqueda
        if termino_busqueda:
            termino_busqueda = termino_busqueda.lower()
            mask = (df['Nombre'].astype(str).str.lower().str.contains(termino_busqueda, na=False) |
                   df['Codigo'].astype(str).str.lower().str.contains(termino_busqueda, na=False) |
                   df['Proveedor'].astype(str).str.lower().str.contains(termino_busqueda, na=False))
            df = df[mask]
        
        # NO filtrar por dueño - permitir que todos los proveedores sean visibles
        # if dueno_filtro:
        #     df = df[df['Dueno'].astype(str).str.lower() == dueno_filtro.lower()]
        
        print(f"Productos encontrados después de filtros: {len(df)}")
        ...
        for _, row in df.iterrows():
            resultado = {
                ...
                'proveedor': str(row['Proveedor']) if pd.notna(row['Proveedor']) else '',
                'precio': float(row['Precio']) if pd.notna(row['Precio']) and str(row['Precio']) != 'nan' else 0.0,
                'precio_texto': str(row['PrecioTexto']) if pd.notna(row['PrecioTexto']) and str(row['PrecioTexto']) != 'nan' else str(row['Precio']) if pd.notna(row['Precio']) else '0',
                'observaciones': str(row['Observaciones']) if pd.notna(row['Observaciones']) else '',
                'dueno': str(row['Dueno']) if pd.notna(row['Dueno']) else 'ferreteria_general',
                'es_manual': True
            }
            resultados.append(resultado)
            print(f"Producto agregado: {resultado['nombre']} - Código: {resultado['codigo']} - Proveedor: {resultado['proveedor']} - Dueño: {resultado['dueno']}")
...
def agregar_producto():
    ...
    proveedores_manuales = db_query("SELECT id, nombre FROM proveedores_manual ORDER BY nombre", fetch=True) or []
    
    # Obtener lista de proveedores Excel disponibles
    proveedores_excel = []
    for key, config in PROVEEDOR_CONFIG.items():
        archivo_excel = os.path.join(EXCEL_FOLDER, f"{key}.xlsx")
    ...
    return render_template('agregar.html', 
                         ...
                         proveedores_manuales=proveedores_manuales,
...
def procesar_archivo_excel(archivo, config, termino_busqueda, filtro_adicional, proveedor_key):
    ...
    try:
        archivo_path = os.path.join(EXCEL_FOLDER, f"{archivo}.xlsx")
